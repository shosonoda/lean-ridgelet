...
- In the private repository, `00data/ghost20260718.pdf` is authoritative for theorem statements,
  constants, ordering, and numbering.
- The active LaTeX flow starts at `../-draft-Ghosts/main.tex` and uses `head.tex`,
  `05journal/02theory02.tex`, `05journal/body.tex`, and `05journal/supp02.tex`. Do not use archived
  `02theory01.tex` or `supp01.tex` as sources of truth. Prefer the PDF if revisions disagree.
...
- Write operator application with explicit brackets in mathematical prose, for example
  `T[γ](x, ω)`, `S_σ[γ](x)`, `R_h[f](x)`, and `R[f; ρ](a, b)`.
- Use `h` for coefficient vectors. Distinguish the coordinate-side maps `\widetilde L` and `J_h`
  from the parameter-space maps `S = \widetilde L T` and `R_h = T* J_h`.

## Documentation policy

- Blueprint prose is English. The target Overview order is the `ghost20260718.pdf` publication
  order beginning with the abstract unitary results; while migration is incomplete, label old
  numbering explicitly as a compatibility map. Organize later chapters by Lean dependency.
