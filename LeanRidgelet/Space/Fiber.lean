...
def fiberBaseNormSq (m : ℕ) (q : SchwartzMap ℝ ℂ) : ℝ :=
  (2 * Real.pi) ^ (m - 1) * ∫ ω : ℝ, ‖q ω‖ ^ 2 * |ω| ^ m

/-- The weighted Bessel-potential term of the fiber norm in Section 2.2. -/
def fiberSobolevNormSq (s t : ℝ) (q : SchwartzMap ℝ ℂ) : ℝ :=
  ∫ ω : ℝ, ‖schwartzBesselPotential t q ω‖ ^ 2 * japaneseBracketPow (-2 * s) ω

/-- The squared norm on the Schwartz core of `H_{s,t}`, before Hilbert completion. -/
def fiberNormSq (m : ℕ) (s t : ℝ) (q : SchwartzMap ℝ ℂ) : ℝ :=
  fiberBaseNormSq m q + fiberSobolevNormSq s t q

theorem fiberBaseNormSq_nonneg (m : ℕ) (q : SchwartzMap ℝ ℂ) :
    0 ≤ fiberBaseNormSq m q := by
  apply mul_nonneg (pow_nonneg (by positivity) _)
  apply integral_nonneg
  intro ω
  positivity

theorem fiberSobolevNormSq_nonneg (s t : ℝ) (q : SchwartzMap ℝ ℂ) :
    0 ≤ fiberSobolevNormSq s t q := by
  apply integral_nonneg
  intro ω
  exact mul_nonneg (sq_nonneg _) (le_of_lt (japaneseBracketPow_pos (-2 * s) ω))

theorem fiberNormSq_nonneg (m : ℕ) (s t : ℝ) (q : SchwartzMap ℝ ℂ) :
    0 ≤ fiberNormSq m s t q := by
  exact add_nonneg (fiberBaseNormSq_nonneg m q) (fiberSobolevNormSq_nonneg s t q)

/-- Polynomially weighted pointwise inner products of Schwartz functions are integrable. -/
theorem integrable_inner_mul_abs_pow (m : ℕ) (q r : SchwartzMap ℝ ℂ) :
    Integrable (fun ω : ℝ ↦ ⟪q ω, r ω⟫_ℂ * (|ω| ^ m : ℝ)) := by
  let C := SchwartzMap.seminorm ℂ 0 0 q
  have hmajor : Integrable (fun ω : ℝ ↦ C * (‖ω‖ ^ m * ‖r ω‖)) :=
    (r.integrable_pow_mul volume m).const_mul C
  apply hmajor.mono
  · fun_prop
  · filter_upwards with ω
    have hC : 0 ≤ C := apply_nonneg _ q
    have hq : ‖q ω‖ ≤ C := SchwartzMap.norm_le_seminorm ℂ q ω
    rw [Real.norm_eq_abs]
...
theorem integrable_inner_mul_japaneseBracketPow (u : ℝ) (q r : SchwartzMap ℝ ℂ) :
    Integrable (fun ω : ℝ ↦ ⟪q ω, r ω⟫_ℂ * japaneseBracketPow u ω) := by
  let wr : SchwartzMap ℝ ℂ := SchwartzMap.smulLeftCLM ℂ (temperedWeight u) r
  have h := integrable_inner_mul_abs_pow 0 q wr
  convert h using 1
  funext ω
...
theorem integrable_norm_sq_mul_abs_pow (m : ℕ) (q : SchwartzMap ℝ ℂ) :
    Integrable (fun ω : ℝ ↦ ‖q ω‖ ^ 2 * |ω| ^ m) := by
  refine (integrable_inner_mul_abs_pow m q q).re.congr ?_
  filter_upwards with ω
...
theorem fiberNormSq_eq_zero_iff (m : ℕ) (_hm : 0 < m) (s t : ℝ) (q : SchwartzMap ℝ ℂ) :
    fiberNormSq m s t q = 0 ↔ q = 0 := by
  constructor
  · intro hnorm
    have hbase : fiberBaseNormSq m q = 0 := by
      have hb := fiberBaseNormSq_nonneg m q
      have hs := fiberSobolevNormSq_nonneg s t q
      unfold fiberNormSq at hnorm
      nlinarith
    have hcoefficient : (2 * Real.pi) ^ (m - 1) ≠ 0 :=
      pow_ne_zero _ (mul_ne_zero (by norm_num) Real.pi_ne_zero)
    have hintegral : ∫ ω : ℝ, ‖q ω‖ ^ 2 * |ω| ^ m = 0 := by
      unfold fiberBaseNormSq at hbase
      exact (mul_eq_zero.mp hbase).resolve_left hcoefficient
    have hae : (fun ω : ℝ ↦ ‖q ω‖ ^ 2 * |ω| ^ m) =ᵐ[volume] 0 :=
      (integral_eq_zero_iff_of_nonneg
        (fun ω ↦ mul_nonneg (sq_nonneg _) (pow_nonneg (abs_nonneg ω) m))
        (integrable_norm_sq_mul_abs_pow m q)).mp hintegral
    have hqae : (q : ℝ → ℂ) =ᵐ[volume] 0 := by
      filter_upwards [hae, volume.ae_ne (0 : ℝ)] with ω hω hω0
      have habs : |ω| ^ m ≠ 0 := pow_ne_zero _ (abs_ne_zero.mpr hω0)
      have hnormSq : ‖q ω‖ ^ 2 = 0 := (mul_eq_zero.mp hω).resolve_right habs
      have hnormZero : ‖q ω‖ = 0 := by
        simpa only [sq_eq_zero_iff] using hnormSq
      exact norm_eq_zero.mp hnormZero
    have hqfun : (q : ℝ → ℂ) = 0 :=
      volume.eq_of_ae_eq hqae q.continuous continuous_zero
    ext ω
...
def fiberInner (m : ℕ) (s t : ℝ) (q r : SchwartzMap ℝ ℂ) : ℂ :=
  (2 * Real.pi : ℂ) ^ (m - 1) *
      (∫ ω : ℝ, ⟪q ω, r ω⟫_ℂ * (|ω| ^ m : ℝ)) +
    (∫ ω : ℝ, ⟪schwartzBesselPotential t q ω, schwartzBesselPotential t r ω⟫_ℂ *
      japaneseBracketPow (-2 * s) ω)

theorem fiberInner_conj_symm (m : ℕ) (s t : ℝ) (q r : SchwartzMap ℝ ℂ) :
    conj (fiberInner m s t r q) = fiberInner m s t q r := by
  have hbase :
      conj (∫ ω : ℝ, ⟪r ω, q ω⟫_ℂ * (|ω| ^ m : ℝ)) =
        ∫ ω : ℝ, ⟪q ω, r ω⟫_ℂ * (|ω| ^ m : ℝ) := by
    rw [← integral_conj]
  ...
  have hsobolev :
      conj (∫ ω : ℝ,
        ⟪schwartzBesselPotential t r ω, schwartzBesselPotential t q ω⟫_ℂ *
          japaneseBracketPow (-2 * s) ω) =
        ∫ ω : ℝ,
          ⟪schwartzBesselPotential t q ω, schwartzBesselPotential t r ω⟫_ℂ *
...
theorem fiberInner_add_left (m : ℕ) (s t : ℝ) (q r u : SchwartzMap ℝ ℂ) :
    fiberInner m s t (q + r) u = fiberInner m s t q u + fiberInner m s t r u := by
  unfold fiberInner
  simp only [add_apply, map_add, inner_add_left, add_mul]
  rw [integral_add (integrable_inner_mul_abs_pow m q u)
    (integrable_inner_mul_abs_pow m r u)]
  rw [integral_add
    (integrable_inner_mul_japaneseBracketPow (-2 * s) (schwartzBesselPotential t q)
      (schwartzBesselPotential t u))
    (integrable_inner_mul_japaneseBracketPow (-2 * s) (schwartzBesselPotential t r)
      (schwartzBesselPotential t u))]
  ring

theorem fiberInner_smul_left (m : ℕ) (s t : ℝ) (q r : SchwartzMap ℝ ℂ) (c : ℂ) :
    fiberInner m s t (c • q) r = conj c * fiberInner m s t q r := by
  unfold fiberInner
  simp only [smul_apply, map_smul, inner_smul_left, mul_assoc]
  rw [integral_const_mul, integral_const_mul]
  ring

theorem fiberInner_self_re (m : ℕ) (s t : ℝ) (q : SchwartzMap ℝ ℂ) :
    (fiberInner m s t q q).re = fiberNormSq m s t q := by
  have hbase :
      (∫ ω : ℝ, ⟪q ω, q ω⟫_ℂ * (|ω| ^ m : ℝ)).re =
        ∫ ω : ℝ, ‖q ω‖ ^ 2 * |ω| ^ m := by
    calc
      _ = ∫ ω : ℝ, RCLike.re (⟪q ω, q ω⟫_ℂ * (|ω| ^ m : ℝ)) :=
        (integral_re (integrable_inner_mul_abs_pow m q q)).symm
      _ = _ := by
        apply integral_congr_ae
        filter_upwards with ω
        rw [RCLike.mul_re, inner_self_eq_norm_sq, inner_self_im, zero_mul, sub_zero]
  ...
  have hsobolev :
      (∫ ω : ℝ,
        ⟪schwartzBesselPotential t q ω, schwartzBesselPotential t q ω⟫_ℂ *
          japaneseBracketPow (-2 * s) ω).re =
        ∫ ω : ℝ, ‖schwartzBesselPotential t q ω‖ ^ 2 *
          japaneseBracketPow (-2 * s) ω := by
    calc
      _ = ∫ ω : ℝ, RCLike.re
          (⟪schwartzBesselPotential t q ω, schwartzBesselPotential t q ω⟫_ℂ *
        ...
        (integral_re (integrable_inner_mul_japaneseBracketPow (-2 * s)
          (schwartzBesselPotential t q) (schwartzBesselPotential t q))).symm
      _ = _ := by
        apply integral_congr_ae
        filter_upwards with ω
        rw [RCLike.mul_re, inner_self_eq_norm_sq, inner_self_im, zero_mul, sub_zero]
...
theorem fiberInner_self_re_nonneg (m : ℕ) (s t : ℝ) (q : SchwartzMap ℝ ℂ) :
    0 ≤ (fiberInner m s t q q).re := by
  rw [fiberInner_self_re]
  exact fiberNormSq_nonneg m s t q

theorem fiberInner_self_eq_zero (m : ℕ) (hm : 0 < m) (s t : ℝ) (q : SchwartzMap ℝ ℂ) :
    fiberInner m s t q q = 0 → q = 0 := by
  intro hq
  apply (fiberNormSq_eq_zero_iff m hm s t q).1
...
theorem denseRange_fiberCore_coe (m : ℕ) [NeZero m] (s t : ℝ) :
    DenseRange (fun q : FiberCore m s t ↦ (q : FiberSpace m s t)) :=
