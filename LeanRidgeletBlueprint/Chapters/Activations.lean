...
If $`t>1/2`, then $`\langle x\rangle^{-t}\tanh x\in L^2(\mathbb R)`, and $`\tanh` has a
nonzero tempered-distribution realization in $`\mathcal A_{0,t}`.
:::

:::proposition "relu_activation" (lean := "LeanRidgelet.memLp_reluWeightedFn, LeanRidgelet.reluTemperedDistribution_apply")
If $`t>3/2`, then $`\langle x\rangle^{-t}x_+\in L^2(\mathbb R)`, and ReLU has a nonzero
tempered-distribution realization in $`\mathcal A_{0,t}`.
:::
