...
published theory subtrees only; that is the whole reason it exists, and it is why a
development-only node cannot reach the public graph and summary.

No subtree is development-only at present -- the harmonic-analysis one joined the published ones on
2026-08-19 -- so this carrier and `LeanRidgeletBlueprint.Generated` currently import the same parts.
Keep them apart: the separation is what makes the next unstable subtree containable.
